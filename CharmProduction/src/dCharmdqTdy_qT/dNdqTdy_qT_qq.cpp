...
int main(int argc, char* argv[]) {
    ...
    double QMin=3.0; double QMax=12;
    
    CommandlineArguments.Getval("QMin",QMin);
    CommandlineArguments.Getval("QMax",QMax);
    
    double qTMin=0; double qTMax=20.0; double TauMin=0.0; double TauMax=40.0;
    ...
    std::cerr << "#KINEMATIC CUTS ARE Q=" << QMin << " - " << QMax << "GeV" << " AND tau=" << TauMin << "-" << TauMax << " fm" << std::endl;
    ...
    // CALCULATE CHARM PRODUCTION -- dN/dqTdy //
    int NQ=300; double yQ=0; double dNchdEta = 2240;
        
    CommandlineArguments.Getval("NQ",NQ);
    CommandlineArguments.Getval("yQ",yQ);
        
    std::cerr << "#CALCULATING FOR qT=" << qTMin << " - " << qTMax << "GeV IN " << NQ << " BINS AT yQ=" <<  yQ  << " WITH " << NSamples << " SAMPLES PER BIN" << std::endl;
    
    // WRITE HEADER //
    std::cout << "#1-qT [GeV] #2--dN_{tot}/dqTdy [GeV-1] #3--dN_{PreEq}/dqTdy [GeV-1] #4--dN_{Hydro}/dqTdy [GeV-1]" << std::endl;
        
    for(int iQ=0;iQ<NQ;iQ++){
    	double qT=qTMin+iQ*(qTMax-qTMin)/double(NQ-1);
    	double dNlldQdY=0.0;
    	...
    	for(int i=0;i<NSamples;i++){
        	...
        	CharmRates_qq::SampledNdqTdy(QMin, QMax,qT,TauMin,TauMax,yQ,dNchdEta,Area,EtaOverS,MQ,alphas,dN,dNPreEq,dNHydro);
        ...
        std::cout << qT << " " << dNlldQdY << " " << dNlldQdYPreEq << " " << dNlldQdYHydro ;
        cout << endl;
        }
         return 0;
}
