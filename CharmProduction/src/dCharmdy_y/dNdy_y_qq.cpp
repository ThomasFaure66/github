...
int main(int argc, char* argv[]) {
 ...
 // COLLISION PARAMETERS //
    double EtaOverS=0.32; double Area=110; double alphas=0.3;
    ...
    std::cerr << "#CALCULATING CHARM/ANTICHARM PRODUCTION FOR  Area=" << Area << " fm^2 AND Eta/s=" << EtaOverS << " QUARK SUPPRESION " << QUARK_SUPPRESSION << " Charm mass " << MQ << " alphas " << alphas << " Centrality " << a << "-"<< b << "%" << std::endl;
    ...
    // WRITE HEADER //
    std::cout << "#1--y #2--dN_{tot}/dy #3--dN_{PreEq}/dy #4--dN_{Hydro}/dy" << std::endl;
   		...
   		cout << yQ << " ";
                ...
                for(int i=0;i<NSamples;i++){
                	double dN, dNPreEq, dNHydro;    
                        CharmRates_qq::SampledNdy(QMin,QMax,qTMin,qTMax,TauMin,TauMax,yQ,dNchdEta,Area,EtaOverS, MQ,alphas, dN, dNPreEq, dNHydro);
