namespace DileptonRates{
    ...
    void SampleTracePic(double Q, double q0,double qT,double PhiQ, double yQ,double EtaX,double Xi,double Teff,double qSupp, double &sampletrace, double &sigma, double &test1){
        ...
        double alphas  = 0.3;
        sigma=((M_PI*alphas*alphas)/(2.0*Q*Q))*(((11.0/4.0)-(3.0/2.0)*v*v+(1.0/12.0)*v*v*v*v)*atanh(v)-(v/24.0)*(59.0-31.0*v*v));
        test1=v;
        // GET PHASE SPACE DISTRIBUTION //
    ...
    void SampledNdQdy(double QSqr,double qTMin,double qTMax,double TauMin,double TauMax,double EtaQ,double dNchdEta,double Area,double etas,double &dN,double &dNPreEq,double &dNHydro, double &sigmatilde, double &test1t){
        ...
        double sampletrace,sigma, test1;
        SampleTracePic(Q,q0,qT,PhiQ,yQ,EtaX,Xi,Teff,qSupp,sampletrace,sigma, test1);
        double PreFactor=QSqr/(64.0*4.0*M_PI*M_PI*M_PI*M_PI*M_PI*M_PI);
        double dNlld4xd4Q=PreFactor*sampletrace;
        sigmatilde = sigma;
        test1t=test1;
